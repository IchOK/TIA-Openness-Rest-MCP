...
using System.Net;
using Tophinke.TiaOpenness.Tool.Types.FC;

namespace Tophinke.TiaOpenness.Tool.TiaREST {
  static internal class cTiaFCs {
    ...
    static public string Get(HttpListenerContext context) {
      ...
      try {
        var retValue = TiaConnectionManager.Instance.ExecuteWithProject(processIdStr, projectName, project => {
          ...
          if (block == null || !(block is FC)) {
            errorMessage = $"Error: FC with name {blockName} not found in PLC software {plcSoftware.Name}.";
            Console.Error.WriteLine(errorMessage);
            context.Response.StatusCode = (int)HttpStatusCode.NotFound;
            context.Response.ContentType = "text/plain";
            return errorMessage;
